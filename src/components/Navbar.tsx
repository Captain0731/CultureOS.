...
export default function Navbar() {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      const threshold = 50; // Threshold for scroll effect
      const shouldBeScrolled = scrollPosition > threshold;

      setIsScrolled(shouldBeScrolled);

      // Toggle body class for content spacing
      if (shouldBeScrolled) {
        document.body.classList.add("with-nav-space");
      } else {
        document.body.classList.remove("with-nav-space");
      }
    };
